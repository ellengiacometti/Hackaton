from manipula_audio import encodeAudio
...
def voz2Texto(nome_audio):
    ...
    with open(join(dirname(__file__), './.', nome_audio),
              'rb') as audio_file:
    ...
    while bool(watson_resultado.get('results')):
        script = watson_resultado.get('results').pop().get('alternatives').pop().get('transcript') + script[:]
    return script
...
def texto2Intencao(script):
    ...
    response = assistant.message(
        ...
        input={'message_type': 'text', 'text': script}).get_result()
...
def montaAudioContasPagas(desc_pagamentos, valor, dia):
    ...
    if len(desc_pagamentos) == 1:
        falafinal = "Olha você já pagou " + valor[0] + " reais  referente a " + desc_pagamentos[
